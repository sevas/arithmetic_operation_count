...
from .common import OpCount, range_to_count, OpCountNode, log_indented
...
def make_opcount_tree(node, level=0) -> OpCountNode:
    if isinstance(node, pycparser.c_ast.BinaryOp):
        log_indented(f"binary op {node.op}", level)
        if node.op == "*":
            oc = OpCount(mul=1)
        elif node.op in ["+", "-"]:
            oc = OpCount(add=1)
        ...
        log_indented("unary op", level)
        return OpCountNode(
            name=node.op, op_count=OpCount(add=1), children=[make_opcount_tree(node.expr, level=level + 1)]
        )

    elif isinstance(node, pycparser.c_ast.Assignment):
        if node.op == "=":
            log_indented("assignment", level)
            return make_opcount_tree(node.rvalue, level=level + 1)
        elif node.op in ["+=", "-="]:
            log_indented("assignment with op", level)
            return OpCountNode(
                name=node.op, op_count=OpCount(add=1), children=[make_opcount_tree(node.rvalue, level=level + 1)]
            )
        elif node.op == "*=":
            log_indented("assignment with op", level)
            return OpCountNode(
                name=node.op, op_count=OpCount(mul=1), children=[make_opcount_tree(node.rvalue, level=level + 1)]
            )
        elif node.op == "/=":
            raise NotImplementedError(node.op)
        else:
            log_indented(f"!!! assignment with op {node.op}", level)
            return OpCountNode(name=node.op, op_count=OpCount(), children=[])

    elif isinstance(node, pycparser.c_ast.Constant):
        log_indented("constant", level)
        return OpCountNode(name="constant", op_count=OpCount(), children=[])

    elif isinstance(node, pycparser.c_ast.ID):
        log_indented(f"ID: {node.name}", level)
        return OpCountNode(name="ID", op_count=OpCount(), children=[])

    elif isinstance(node, pycparser.c_ast.FuncDef):
        log_indented("Func Body", level)
        log_indented(f"recurse in {node.body.__class__.__name__}", level)
        ...
        return OpCountNode(
            ...
            is_branch=True,
            children=[
                make_opcount_tree(node.iftrue, level=level + 1),
                make_opcount_tree(node.iffalse, level=level + 1),
            ],
        )

    elif isinstance(node, pycparser.c_ast.Decl):
        log_indented(f"recurse in {node.init.__class__.__name__}", level)
        if node.init:
        ...
        for each in node.children():
            log_indented(f"recurse in {each[1].__class__.__name__}", level)
            return make_opcount_tree(each[1], level=level + 1)

    else:
        # all other case, we forward to child AST nodes
        # raise NotImplementedError(node.__class__.__name__)
        log_indented(f"!!! {node.__class__.__name__}", level)
        for each in node.children():
            log_indented(f"recurse in {each[1].__class__.__name__}", level)
            return make_opcount_tree(each[1], level=level + 1)
